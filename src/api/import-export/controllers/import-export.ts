...
type ConfidenceMapping =
  | { action: "set"; key: ConfidenceKeyValue }
  | { action: "clear" }
  | { action: "invalid"; reason: string };

interface ConfidenceKeyImportStats {
  total: number;
  matched: number;
  updated: number;
  unchanged: number;
...
function parseImportAction(ctx: Context): "apply" | "dry-run" {
...
function normalizeStoredConfidenceKey(value: unknown): string | null {
  if (value === undefined || value === null || value === "") return null;
  return String(value);
}

function logConfidenceImport(
  strapi: any,
  message: string,
...
function stripNotionUrlSuffix(value: string): string {
  return value.replace(/\s*\(https?:\/\/[^\)]*\)?$/i, "").trim();
}

function parseResearchPaperTitles(raw: string): string[] {
...
function stripEmojiPresentation(value: string): string {
 ...
 */
function normalizeConfidenceCell(raw: string): string {
...
function confidenceContains(value: string, emoji: string): boolean {
  return stripEmojiPresentation(value).includes(stripEmojiPresentation(emoji));
}

function confidenceContainsAny(value: string, emojis: string[]): boolean {
  return emojis.some((emoji) => confidenceContains(value, emoji));
}

function isMemoOnlyConfidence(value: string): boolean {
  const stripped = stripEmojiPresentation(value).replace(/[\s,;|/\\-]+/g, "");
  if (!stripped) return true;
  return /^🗒+$/.test(stripped);
}

function mapConfidenceValueToKey(raw: string): ConfidenceMapping {
  const value = normalizeConfidenceCell(raw);
  if (!value) return { action: "clear" };

  if (VALID_CONFIDENCE_KEYS.has(value)) {
    return { action: "set", key: value as ConfidenceKeyValue };
  }
  ...
  if (hasFlag) return { action: "set", key: "caution" };
  if (hasCap) return { action: "set", key: "cap_evidence" };
  if (hasStar) return { action: "set", key: "star_evidence" };
  if (hasCheck) return { action: "set", key: "check_evidence" };

  if (hasMemo) return { action: "clear" };
...
function resolveImportRowKey(record: Record<string, string>): string {
  const resultTitle = getRowValue(record, "Results").trim();
  const paperTitle =
    parseResearchPaperTitles(getRowValue(record, "Research Papers", "Title"))[0] ||
    "";
...
async function findResultsForConfidenceImport(
  ...
  if (resultTitle) {
    const byTitle: any = await strapi
  ...
  if (candidates.length === 0 && paperTitles.length > 0) {
    for (const paperTitle of paperTitles) {
      ...
      if (!papers?.[0]?.id) continue;

      const linked: any = await strapi.documents("api::result.result").findMany({
        ...baseQuery,
  ...
  if (paperTitles.length > 0 && uniqueCandidates.length > 1) {
    const narrowed = uniqueCandidates.filter((result) =>
      (result.researchPapers ?? []).some((paper: { Title?: string }) =>
        paperTitles.some(
          (title) =>
            (paper.Title ?? "").trim().toLowerCase() === title.toLowerCase()
        )
      )
    );
...
async function importResultConfidenceKeys(
  ...
  logConfidenceImport(
    strapi,
    apply ? "Starting APPLY import" : "Starting DRY-RUN import",
    { total: records.length }
  );
  ...
  if (apply) {
    logConfidenceImport(strapi, "APPLY import finished", {
      planned_updates: stats.planned_updates ?? 0,
      write_success: stats.write_success ?? 0,
      write_failed: stats.write_failed ?? 0,
      verification,
    });
  }
  ...
  async import(ctx: Context) {
    try {
      ...
      if (collection === "supplements") {
        ...
        collection === "result-confidence-key" ||
        ...
        strapi.log.info(
          `[import-export] result-confidence-key import received action=${action} apply=${apply} dryRun=${!apply}`
        );
        ...
        } = await importResultConfidenceKeys(strapi, csvRecords, apply);

        const writeSuccess = confidenceStats.write_success ?? 0;
        const writeFailed = confidenceStats.write_failed ?? 0;
        const plannedUpdates = confidenceStats.planned_updates ?? 0;
