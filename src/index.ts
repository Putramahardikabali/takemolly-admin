...
export default {
  ...
  async bootstrap({ strapi }) {
    ...
    //   SEED_TARGET=results
    const targetEnv = (process.env.SEED_TARGET ?? "all").toLowerCase();
    ...
    if (!wantResults && !wantPapers && !wantSupplements && !wantBiomarkers) {
      console.log(
        `⚠️  SEED_TARGET="${targetEnv}" matched nothing. Valid: all, results, papers, supplements, biomarkers`
      );
      return;
    }

    console.log(
      `🌱 SEED_TARGET=${targetEnv} → results:${wantResults} papers:${wantPapers} supplements:${wantSupplements} biomarkers:${wantBiomarkers}`
    );
    ...
    try {
      ...
      const mustClearResults =
        wantResults || wantPapers || wantSupplements;
      ...
      try {
        ...
        if (wantPapers) {
          const papers = await strapi.entityService.findMany(
        ...
        if (wantSupplements) {
          const supplements = await strapi.entityService.findMany(
        ...
        if (wantBiomarkers) {
          const biomarkers = await strapi.entityService.findMany(
      ...
      if (wantBiomarkers) {
        const { seedBiomarkers } = await import("./seeders/biomarkers");
        console.log("\n🌱 Seeding biomarkers...");
        await seedBiomarkers(strapi);
      }

      if (wantSupplements) {
        const { seedSupplements } = await import("./seeders/supplements");
        console.log("\n🌱 Seeding supplements...");
        await seedSupplements(strapi);
      }

      if (wantPapers) {
        const { seedResearchPapers } = await import(
          "./seeders/research-papers"
        );
      ...
      if (wantResults) {
        const { seedResults } = await import("./seeders/results");
