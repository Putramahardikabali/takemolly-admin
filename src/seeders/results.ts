...
function extractNotionSlug(
  ...
  if (!extractNotion) return slugify(value);
...
function buildDynamicMap(csvHeaders: string[]): Record<string, string> {
  ...
  for (const rawHeader of csvHeaders) {
    ...
    const mapped = HEADER_SCHEMA[header];
...
type CachedEntity = { id: number; slug: string };

interface EntityCache {
  supplements: CachedEntity[];
  supplementBySlug: Map<string, CachedEntity>;
  papers: CachedEntity[];
  paperBySlug: Map<string, CachedEntity>;
}

async function loadEntityCache(strapi: any): Promise<EntityCache> {
  ...
  const dedupe = (rows: any[]): CachedEntity[] => {
...
function findSupplementId(
  cache: EntityCache,
  csvSlug: string
): { id: number; matchedSlug: string; fuzzy: boolean } | null {
  if (!csvSlug) return null;
  const exact = cache.supplementBySlug.get(csvSlug);
  if (exact) return { id: exact.id, matchedSlug: exact.slug, fuzzy: false };
...
function extractPaperTitleSlugs(raw: string | undefined | null): string[] {
  if (!raw) return [];
  const refs: string[] = [];
  ...
  while ((m = re.exec(raw)) !== null) {
    ...
    if (title) refs.push(slugify(title));
  }
...
function extractSupplementSlugs(raw: string | undefined | null): string[] {
  if (!raw) return [];
...
export const seedResults = async (strapi: any) => {
  console.log("=== Starting Strapi Results Seeder ===");

  // Load CSV raw text - Strapi runs from project root
  const csvPath = path.join(process.cwd(), "src/seeders/csv/results.csv");
  ...
  const builtRows: StrapiRow[] = records.map((csvRow) =>
    mapCsvRowToStrapiRow(csvRow, csvToStrapiMap)
  );
  ...
  for (const r of builtRows) {
    r.slug = makeUniqueSlug(r.slug as string);
  }
  ...
  const insertedIds: (number | null)[] = new Array(builtRows.length).fill(null);
  ...
  for (let i = 0; i < builtRows.length; i++) {
    ...
    try {
      ...
      console.error(
        "Row data:",
        JSON.stringify(records[i]).substring(0, 200)
      );
    }
  }
  ...
  console.log(
    `📦 Cache: ${cache.supplements.length} supplements, ${cache.papers.length} papers`
  );
  ...
  let suppFuzzy = 0;
  let suppMissing = 0;
  const missingPaperSlugs = new Set<string>();
  const missingSuppSlugs = new Set<string>();
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    try {
      // Research papers
      const paperSlugs = extractPaperTitleSlugs(
        csvRow["Research Papers"] || ""
      );
      ...
      relationFailed++;
      console.error(
        `❌ Relation update failed for ${strapiRow.slug}:`,
        (err as Error).message
      );
    }
  }
  ...
  if (missingSuppSlugs.size > 0) {
    console.log(
      "\n⚠️  Unmatched supplement slugs (sample):",
      Array.from(missingSuppSlugs).slice(0, 20).join(", ")
    );
  }
  if (missingPaperSlugs.size > 0) {
    console.log(
      "\n⚠️  Unmatched paper slugs (sample):",
      Array.from(missingPaperSlugs).slice(0, 20).join(", ")
    );
  }
};
