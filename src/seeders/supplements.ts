...
function mapCsvRowToStrapiRow(
  ...
  strapiRow.slug = extractNotionSlug(strapiRow.supplement_name as string);
  strapiRow.symbol = SUPPLEMENT_SYMBOLS[strapiRow.slug as string] || "";
  strapiRow.color = SUPPLEMENT_COLORS[strapiRow.slug as string] || "#FFD465";
  strapiRow.border_color =
    SUPPLEMENT_BORDER_COLORS[strapiRow.slug as string] ||
    "rgba(234, 145, 248, 0.8)";
  strapiRow.is_our_supplement = SUPPLEMENT_SYMBOLS[strapiRow.slug as string]
    ? true
    : false;
  strapiRow.description =
    SUPPLEMENT_DESCRIPTION[strapiRow.slug as string] || "";

  return strapiRow;
}

export const seedSupplements = async (strapi: any) => {
  console.log("=== Starting Strapi Supplements Seeder ===");

  // Load CSV raw text - Strapi runs from project root
  const csvPath = path.join(process.cwd(), "src/seeders/csv/supplements.csv");
...
export default seedSupplements;
